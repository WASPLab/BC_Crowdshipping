...
const getAttributeData = async (org) => {
    let attributes = null
    org == 'Org1' ? attributes = {key: "role", value: 'Seller'} : null
    org == 'Org2' ? attributes = {key: "role", value: 'Customer'} : null
    org == 'Org3' ? attributes = {key: "role", value: 'Courier'} : null
    return attributes
}
...
const getRegisteredUser = async (username, userOrg, isJson) => {
    ...
    let attribute = await getAttributeData(userOrg)
    ...
    try {
        ...
        secret = await ca.register({ affiliation: 'org1.department1', enrollmentID: username, role: 'client', attrs: [{ name: attribute.key, value: attribute.value, ecert: true }] }, adminUser);
    ...
    const enrollment = await ca.enroll({ enrollmentID: username, enrollmentSecret: secret, attr_reqs: [{ name: attribute.key, optional: false }] });
