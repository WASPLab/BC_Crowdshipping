    ...
    try {
        ...
        switch (fcn) {
            case "QueryOrder":
                 ...
                 result = {Order: JSON.parse(result.toString())}
                break; 

            case "QueryParcel":
                result = await contract.evaluateTransaction(fcn, args[0]);
                result = {Parcel: JSON.parse(result.toString())}
                break; 
             
            case "QueryOrderPrivateDetails":
            ...
            case "QueryBid":
                result = await contract.evaluateTransaction(fcn, args[0], args[1]);
                result = {Bid: JSON.parse(result.toString())}
                break; 
            
            case "QueryOrderByState":
                result = await contract.evaluateTransaction(fcn, args[0]);
                result = {Orders: JSON.parse(result.toString())}
                break;  
